...
for S in PDF417 CODE128 EAN13; do
  ...
  if awk -v a="${r:-0}" 'BEGIN{exit !(a < 75.0)}'; then DENSE_BAD=1; fi
done
# [하한이 75%인 이유]
# 예전에는 50%였다. 세로로 이웃한 같은 내용 코드를 isBandOfSameCode가
# 합쳐버리는 것을 못 고쳤기 때문이다 — 그 상한을 두께로 좁히면 밀집이
# 살아나는 대신 **중복이 0 -> 2건**이 됐고, 두 상황이 기하로는 구분이
# 안 된다는 것을 상자를 나란히 놓고 확인했다.
#
# 고쳤다. 기하 대신 **프레임이 이미 증명한 것**으로 가른다 — 느슨한
# 판정으로도 같은 텍스트가 2개 이상 살아남은 프레임에서만 좁은 상한을
# 쓴다(pipeline.cpp의 dedup() 주석, [[vscan-lite-dedup-band-need]]).
# 실측(이 단계 그대로):
#   PDF417   85.0% -> 100.0%    <- 납작해서 이 규칙에 제일 크게 물렸다
#   CODE128  79.4% ->  79.4%    <- 안 변한다(이 규칙이 원인이 아니었다)
#   EAN13    87.5% ->  87.5%
# 중복은 300장 코퍼스에서 0을 유지한다.
#
# 하한을 79.4%보다 낮은 75%로 두는 이유: CODE128/EAN13의 남은 손실은
# dedup이 아니라 밀집 프레임 자체의 디코딩 한계이고, 그 값이 실행마다
# 소폭 흔들린다. 여기서 잡으려는 것은 "dedup이 다시 코드를 먹기 시작했나"
# 이고, 그건 85 -> 50대로 떨어지는 모양으로 나타난다.
echo "   밀집(코드 4~16개):$DENSE_LINE  (하한 75%)"
if [ "$DENSE_BAD" != "0" ]; then
