...
typedef struct {
     ...
     *   DataMatrix 모듈 3px  3/10 -> 5/10
     *
     * **대가는 꼬리 지연이다.** 난수 300장 p95는 302.8 -> 304.9ms로 거의
     * 안 변하는데, 기준 작업량으로 정규화한 회귀 게이트에서는 139.3 ->
     * 174.0으로 25% 회귀다. 원래 빨리 포기하던 느린 프레임이 이제 영역을
     * 찾아 정밀 단을 마저 돌기 때문이다. 그래서 기본은 꺼짐이다.
     *
