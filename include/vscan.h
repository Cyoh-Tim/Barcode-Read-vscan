...
typedef struct {
     ...
     *
     * [!! 그 -8%는 회전 축을 심하게 과소평가한다 — 2026-08-03 추가 실측]
     * 이 옵션이 끄는 것 중에 **영역 구제**가 있는데, 그게 정확히 회전된
     * 코드를 담당한다(§3.17: 회전이 문제가 아니라 탐색 면적이 문제이고,
     * 코드 주변만 잘라내면 각도와 무관하게 읽힌다). 0~90도 스윕:
     *
     *   심볼로지      기본 -> fast_no_read
     *   QR           100% -> **100%**   <- 파인더가 회전 불변이라 안 잃는다
     *   DataMatrix   100% ->  57.9%
     *   EAN13        100% ->  47.4%
     *   ITF          100% ->  26.3%
     *   CODE39       100% ->  21.1%
     ...
     *   PDF417       100% ->  10.5%
     *
     * 오디코딩은 전부 0이다(정확성 문제가 아니라 검출 문제다).
     *
     * **즉 이 옵션은 사실상 QR 전용이다.** QR은 1도 간격 91각도에서
     * `NO_INVERT`까지 얹어도 100%를 지킨다(module 8). 다만 모듈이 작아지면
     * QR도 조금 잃는다(module 3에서 94.7%).
     *
     * 회전된 1D/PDF417이 들어오는 배치에서는 **켜지 말 것.**
     *
