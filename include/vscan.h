 ...
 *   저조도(QR만)   코드 30 -> 30(동일), 판정 최대 18.6 -> 10.9ms
 *   난수 200장     코드 232 -> 220(-5%), 판정 최대 70.6 -> 35.0ms
 ...
 *
 * 실측(2단계 + fast_no_read, reps 3):
 *   저조도(QR만)   코드 30 -> 30(동일),  판정 최대 18.6 -> 4.4ms
 *   난수 200장     코드 232 -> **122**,  판정 최대 70.6 -> 7.7ms
 *
 * **난수 코퍼스에서 검출이 절반으로 떨어진다.** 저조도에서 공짜로 보이는
 * 것은 그 코퍼스가 QR 단일에 마스크까지 좁힌 조건이라 그렇다.
