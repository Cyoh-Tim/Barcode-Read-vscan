...
typedef struct {
     ...
     * [개수를 모를 때도 값이 있다 — 2026-08-03 정정]
     * 예전에는 "개수를 모르는 현장이라면 이 옵션으로는 부분 검출을 못
     * 막는다"고 적혀 있었다. **실측으로 틀렸다.** 개수를 몰라도 **크게
     * 잡아두는 것**이 0보다 훨씬 낫다. 다만 코퍼스에 따라 방향이 갈리므로
     * 자기 프레임에서 재고 정할 것:
     *
     *   코퍼스                 개수 0        고정 99
     ...
     *   40종(대부분 단일 코드)  59/60        56/60        (아래 정정 참고)
     *   저조도 24장             30/48        30/48        (같음, 시간 2배)
     *
     * [단일 코드 손해는 라이브러리 버그였다 — 2026-08-04 재정정]
     * 위 표의 마지막에서 둘째 줄(40종 59 -> 56)을 "값을 크게 주면 치르는
     * 대가"로 설명했었다. **틀렸다.** 영역 구제에 "찾은 영역이 요구 개수
     * 보다 적으면 통째로 건너뛴다"는 검사가 있어서, 힌트가 실제 개수보다
     * 크면 코드가 하나뿐인 프레임의 구제가 아예 안 돌았던 것이다(§3.86).
     * 고친 뒤 실측:
     *
     *   min_expected_codes    0     2     6    12    99
     *   40종(코드 60개)      60/60 60/60 60/60 60/60 60/60
     *
     * **어느 값에서도 안 진다.** 난수 코퍼스(씨드 11, 코드 490개)에서도
     * 값 6이 301 -> 319, 값 12가 301 -> 320으로 올랐다.
     *
     * 남는 대가는 **시간뿐**이다. 값을 크게 주면 각 단계가 더 늦게
     * 만족하므로 그만큼 더 찾는다 — 그건 이 옵션의 계약 그대로다.
     *
     * [2026-08-04: full 경로가 이 옵션을 아예 안 보고 있었다 — §3.88]
     * `vscan_process_gray()`(full 경로)는 **코드 하나만 찾으면 반환**했다.
     * 즉 이 옵션은 2단계/추적 경로에서만 동작했다. 고친 뒤 실측
     * (난수 4시드 2083코드, 개수를 정확히 준 조건):
     *
     *   코드 1435 (68.9%) -> **1494 (71.7%)**, 오디코딩 2 -> 2(그대로),
     *   평균 86 -> 118ms
     *
     * **개수를 0으로 두면 동작이 완전히 같다** — 현장 조건 코퍼스에서
     * 220/550 / 47.6ms 대 47.8ms로 소수점까지 같았다. 즉 이 옵션을 안
     * 쓰는 배치의 지연은 전과 동일하다.
     *
     * 정확한 개수를 아는 것이 여전히 가장 좋다. 모르면 "이 라인에 최대
     * 몇 개까지 올 수 있나"라는 현실적 상한을 넣을 것.
     */
    ...
    /*
     * [로케이터 되살리기] 0=기본(끔) / 1=켬.
     *
     * 노이즈가 큰 프레임에서 후보 영역이 **프레임 전체 한 덩어리**로 뭉개질
     * 때, 임계를 올려 다시 찾는다. 로케이터의 임계가 "프레임 최대 타일
     * 에너지 대비 비율"이라, 센서 노이즈가 배경을 전부 임계 위로 올리면
     * 작은 코드 하나짜리 프레임에서도 영역이 하나로 합쳐진다. 그 크롭은
     * 원본과 같아서 정밀 단이 아무것도 못 번다.
     *
     * 실측(모듈 3px, 노이즈 25): DataMatrix 60x60과 PDF417 525x39가 둘 다
     * 영역 2048x1536(=프레임 전체)으로 잡힌다. 같은 코드를 노이즈 0으로
     * 만들면 256x256으로 정확히 잡히므로, 임계가 아니라 노이즈가 상대
     * 랭킹을 무너뜨린 것이다. **디코더는 멀쩡하다** — 정확한 좌표를 주면
     * 원본 배율 그대로 읽힌다.
     *
     * 켰을 때 (노이즈 0~45 열 단):
     *
     *   PDF417 모듈 2px      2/10 -> 6/10
     *   PDF417 모듈 3px      4/10 -> 9/10
     *   PDF417 모듈 4px      6/10 -> 10/10
     *   DataMatrix 모듈 3px  3/10 -> 10/10
     *   (QR은 원래 되던 자리라 안 바뀐다 — 이 구제가 겨냥한 곳이 아니다)
     *
     * 되살리는 로케이트는 **타일 16 / 축소 2**로 돈다. 임계만 올리면 상자가
     * 타일 격자만큼 헐렁해서(60x60 코드가 256x256 상자) 그 크롭이 다시
     * 실패한다 — 실측에서 같은 코드가 192x192에서는 읽히고 256x256에서는
     * 안 읽혔다. 노이즈 프레임에서는 코드 밖 면적이 곧 오검출 기회다.
     *
     * **대가는 꼬리 지연이다.** 난수 300장 실측:
     *
     *   끔  검출 223/550  평균 55.1ms  p95 264.2ms
     *   켬  검출 223/550  평균 58.0ms  p95 305.6ms   (+5% / +16%)
     *
     * 이 코퍼스는 코드가 크고 여럿이라 **검출은 하나도 안 늘고 지연만 는다.**
     * 회귀 게이트에서도 기준 작업량 대비 p95가 139.3 -> 174.0으로 25%
     * 회귀해서 떨어졌다. 원래 빨리 포기하던 느린 프레임이 이제 영역을 찾아
     * 정밀 단을 마저 돌기 때문이다. 그래서 기본은 꺼짐이다.
     *
     * 켤 만한 배치: 코드가 작고(모듈 4px 이하) 조명이 어두워 노이즈가 큰
     * 라인. **max_frame_ms와 같이 걸 것** — 이 구제는 늘어난 지연을 스스로
     * 자르지 않는다.
     */
    int enable_locate_escalation;
    ...
    /*
     * [개수를 몰라도 남은 코드를 찾는다] 0 = 기본(끔), 1 = 켬.
     *
     * `min_expected_codes`는 "코드가 몇 개인지 안다"를 전제한다. 모르면
     * 파이프라인은 **하나 찾고 끝낸다** — 그게 계약이니 맞는 동작인데,
     * 같은 코퍼스가 개수를 주면 75.3%, 안 주면 40.0%다(코드 단위 텍스트
     * 일치, 씨드 101 300장). 35%p가 순전히 "더 안 찾아서"다.
     *
     * 이 손잡이는 개수를 **프레임에서 추정한다.** 에너지 로케이터가 준
     * 후보 영역 중 이미 읽은 코드로 설명되지 않는 것이 남아 있으면,
     * 그만큼을 기대 개수로 잡고 계속 찾는다. 남는 영역이 없으면(= 코드
     * 하나짜리 프레임) 추정값이 1이라 **동작도 지연도 지금과 같다.**
     *
     * 고정값 추측(`min_expected_codes = 6` 같은)과 다른 점이 그것이다.
     * 고정값은 단일 코드 프레임에서도 끝까지 찾느라 시간을 쓴다.
     *
     * 대가는 **로케이터 한 번**(실측 약 4ms)과, 후보가 남은 프레임에서의
     * 추가 탐색 시간이다.
     *
     * [반드시 `max_frame_ms`와 같이 쓸 것 — 실측이 그렇게 말한다]
     * 이 손잡이는 "더 찾아라"라고만 하고 언제 멈출지는 말하지 않는다.
     * 마감을 같이 걸면 **지금 기본보다 셋 다 낫다**(씨드 101 300장,
     * 2단계, 개수 안 줌, 오디코딩 전부 0):
     *
     *   auto_expected  max_frame_ms   일치율   평균     p95
     *        끔            없음        40.0%   56.1ms   322ms   <- 기본
     *      **켬**        **60ms**    **62.7%** 42.6ms    86ms
     *        켬            없음      **73.6%** 176.9ms   440ms
     *
     * 검출은 마감 30~150ms 구간에서 거의 평평하다(63.6~64.0%). 73.6%까지
     * 가려면 마감을 아예 풀어야 하는데 그 마지막 10%p가 평균을 4배로
     * 만든다. **마감은 절대 벽시계다** — 이 저장소 기준 보드가 x86의 약
     * 8배 느리므로 위 값을 그대로 넣으면 안 된다.
