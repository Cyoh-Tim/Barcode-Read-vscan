...
typedef struct {
     ...
     *   40종(대부분 단일 코드)  59/60        **56/60**    (-3, 나빠진다)
     *   저조도 24장             30/48        30/48        (같음, 시간 2배)
     *
     * 다중 코드 프레임이 많으면 크게 이기고, **단일 코드 프레임에서는
     * 진다.** 지는 이유는 `need`를 못 채우니 모든 단계가 끝까지 돌고,
     * 그러다 프레임 예산에 걸려 **원래 성공했을 깊은 단계가 잘리기**
     * 때문이다. 시간도 같이 늘어난다(40종 8.5 -> 65ms).
     *
     * 정확한 개수를 아는 것이 여전히 가장 좋다(같은 난수 243장에서
     * 480/670). 모르면 "이 라인에 최대 몇 개까지 올 수 있나"를 넣을 것 —
     * 무한대가 아니라 **현실적 상한**이 맞다.
     */
