 ...
 *
 * [기본 OFF — 비용이 크다] 켜면 ROI마다 네 방향(가로/세로 x 정/역)으로
 * 주사선을 훑고 위치마다 네 가지 열 수를 다 시도한다. GS1 Composite의
 * CC-C(PDF417 기반)까지 보므로 훑기가 한 벌 더 붙는다.
 *
 * 실측으로 두 숫자가 다르고 **둘 다 사실이다**:
 *   - 심볼이 바로 읽히는 프레임(MicroPDF417 시험셋 49장): +4%
 *   - 난수 코퍼스 120장(대부분 실패해서 구제 사다리가 디코더를 여러 번 부름):
 *     평균 330 -> 740ms (**2.2배**). 그중 CC-C 몫은 25ms뿐이고 나머지는
 *     MicroPDF417 훑기다.
 * 검출/오디코딩은 두 경우 다 변화 없다(175/344 동일, 오디코딩 동일).
 *
 * 즉 **이 심볼로지를 실제로 쓰는 배치에서만 켤 것.** 줄이려면 훑는 방향을
 * 배치에 맞게 좁히는 것이 가장 크다(예: 세로/역방향을 안 보면 절반).
 *
