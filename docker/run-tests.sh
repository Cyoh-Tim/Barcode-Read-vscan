...
if echo "$LAST" | grep -q "37/40.*37/40.*37/40"; then
  echo ">> ✅ 통과: 세 경로 모두 37/40 — x86/실기 기준선과 동일 (PROJECT_NOTES §7)"
else
  echo ">> ❌ 기준선(37/40) 미달 — $OUT/verify-result.txt 를 확인하세요"
  exit 1
fi
