...
std::vector<PipelineResult> Pipeline::processView(const GrayView& image) {
    ...
    if (cfg_.enableDPMRescue && !partialFrame) {
        ...
        // 걸면 작은 DPM 코드가 열릴 줄 알았다. 15장 표본에서는 34 -> 35로
        // 보였는데, **58프레임 262코드로 키우니 149/262로 완전히 동일**했다.
        // 형태학 커널도 3/5/7/9/11로 스윕했지만 3~7이 전부 같은 값이다.
        // 즉 이 코퍼스의 DPM 실패는 "점을 못 이어붙여서"도 "못 찾아서"도
        // 아니다. 원인 미규명으로 남긴다(§3.84).
    }
...
std::vector<PipelineResult> finalizeConfidence(std::vector<PipelineResult>&& h) {
    ...
    /*
     * [문법이 약한 심볼로지를 **단독일 때도** 버려 봤다 — 되돌렸다]
     *
     * 강등 규칙은 "끝까지 이것뿐이면 그대로 돌려준다"이다. 풀테스트에서
     * 오디코딩 86건 중 54건이 ITF이고 대부분이 "진짜 코드는 못 찾고 유령만
     * 단독으로 나온" 경우라, 체크디짓이 선택인 심볼로지는 단독이어도
     * 버리는 쪽을 재봤다.
     *
     * 실측(씨드 101/11, 각 9000장, 코드 29,892개):
     *
     *   지금(단독이면 남김)   12,766코드   오디코딩 23
     *   단독이어도 버림       12,452코드   오디코딩 21
     *
     * **검출을 314코드 내주고 오디코딩은 2건 줄었다**(n=23의 잡음 안).
     * 값을 못 한다.
     *
     * 왜 안 줄었는지가 이 실측의 소득이다 — 그 ITF 유령들이 대부분
     * lowConfidence로 **표시되지도 않는다**. 즉 좁은 요소가 2px 이상인
     * 판본에서 나온다. §3.107에서 계측한 1.0px 사례는 한 장이었고, 그것을
     * 전체의 기전으로 일반화한 것이 틀렸다. 유령의 기전은 나이퀴스트 미달
     * 하나가 아니다.
     */
...
bool isBandOfSameCode(const BBox& a, const BBox& b, bool tight = false) {
    auto band = [tight](double aLo, double aHi, double bLo, double bHi,   // 긴 축
         ...
         */
        /*
         * [두께로 좁혀 봤고, 되돌렸다 — 기하로는 구분이 안 된다]
         *
         * 이 상한을 `max(ta, tb)`(두께)로 좁히면 밀집 프레임에서 세로로
         * 이웃한 별개 코드를 안 먹는다. 실측: 4x4 PDF417 16개가 8/16 ->
         * 16/16이 됐다. 그런데 **중복이 0 -> 2건**이 됐고 게이트가 잡았다.
         *
         * 두 상황의 상자를 나란히 놓으면 왜 못 가르는지 보인다:
         *
         *              밀집 이웃(별개 코드)      중복 쌍(한 코드의 두 스캔)
         *   상자       684x35 / 684x35           238x24 / 239x21
         *   간격/길이  99%                       79%
         *   합집합비   109%                      98%
         *
         * 같은 텍스트, 같은 긴축 범위, 두께도 비슷하다. 간격 비율(0.35/0.5/
...
bool isStackedBand(const BBox& a, const BBox& b) {
    ...
    auto stacked = [](double aLo, double aHi, double bLo, double bHi,   // 층이 지는 축
         ...
         */
        const bool alongLength =
            (aHi - aLo) > (cHi - cLo) || (bHi - bLo) > (dHi - dLo);
        if (alongLength) {
            // 길이 방향으로 층이 지는 경우가 아예 없지는 않다 — zxing이 한
            // 코드를 길이 방향으로 두 조각으로 끊어 줄 때가 있다. 그것까지
            // 막으면 중복이 생긴다(게이트 실측: 0 -> 2건).
            //
            // 가르는 것은 **간격**이다. 한 코드의 두 조각은 코드 두께 안쪽
            // 간격으로 붙어 있고, 별개 코드는 그보다 훨씬 멀다:
            //   같은 코드의 두 조각   간격 <= 두께 (수 모듈)
            //   4x4 밀집의 이웃 코드  간격 264px = 두께 35px의 7.5배
            const double maxT = std::max(cHi - cLo, dHi - dLo);
            if (gap > maxT) return false;
        }
