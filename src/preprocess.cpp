...
bool flattenIllumination(const GrayView& src, int radius, GrayImage& out, int minSpread) {
    ...
    //
    // [주의: 이 선택은 **x86에서만** 쟀다]
    // 아래 기각은 x86 캐시(L1D 32~48KB, L2 512KB~1MB, 하드웨어 프리페치가
    // 강하다) 기준이다. 타겟인 Cortex-A53은 L1D가 32KB로 작고 L2도
    // 512KB급이라 **64KB 표가 x86보다 불리할 수 있다.** 이 컨테이너에
    // qemu가 없어 aarch64 실행 측정을 못 했다(크로스 컴파일과 기계어
    // 확인까지만 했다 — SIMD 26개 생성 확인).
    // 보드에서 재볼 여지가 있는 자리이고, 되돌리는 것은 아래 기각 기록의
    // 코드를 그대로 쓰면 된다.
    //
    // [1KB 역수표로 바꿔봤고 더 느렸다 — 기각(x86)]
